...
class Hyperbolic(GenModel):
	...
	def ycal(self,x:numpy.ndarray):
		...
		q = q0 / (1+b*D0*t)**(1/b)
		"""
		return self.y0/(1+self.expo*self.base(x))**(1/self.expo)

	def ycum(self,x:numpy.ndarray):
		"""
		Np = q0 / ((1-b)*D0)*(1-(1+b*D0*t)**(1-1/b))
		"""
		return (self.y0/self.D0)/(1-self.expo)*(1-(1+self.expo*self.base(x))**(1-1/self.expo))
	...
	def params(self,x:numpy.ndarray,yobs:numpy.ndarray):
		...
		try:
			LinregressResult = linregress(x,numpy.power(1/yobs,self.expo))
		except Exception as exception:
			logging.error("Error occurred: %s", exception)
			return 0.,0.,None

		y0 = LinregressResult.intercept**(-1/self.expo)
		D0 = LinregressResult.slope/LinregressResult.intercept/self.expo
